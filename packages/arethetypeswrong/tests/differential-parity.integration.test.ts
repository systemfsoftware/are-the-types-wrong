        ...
        Effect.map((exit) =>
          `${entry.name} ${
            Match.value(exit).pipe(
              Match.tag('Success', (succeeded) => JSON.stringify({ tag: 'ok', document: succeeded.value })),
              Match.tag('Failure', (failed) => JSON.stringify({ tag: 'failure', cause: String(failed.cause) })),
              Match.exhaustive,
            )
          }`
        ),
